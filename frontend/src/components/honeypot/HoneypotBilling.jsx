...
export default function HoneypotBilling({ sessionId }) {
    ...
    <div className="space-y-6 animate-in fade-in duration-700">
      <header className="flex justify-between items-end">
        ...
        <div className="flex gap-3">
          <button onClick={() => logHp("Attempted to export financial data", "/admin/payments/export")} className="bg-slate-100 hover:bg-slate-200 text-slate-700 px-6 py-2.5 rounded-xl text-sm font-bold transition-all">
            Export CSV
          </button>
          <button onClick={() => logHp("Attempted to create new invoice", "/admin/payments/write")} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-xl text-sm font-bold transition-all shadow-lg shadow-blue-200">
            Create Invoice
          </button>
        </div>
      </header>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-100">
            {invoices.map((inv) => (
              <tr key={inv.id} onClick={() => logHp(`Inspected Invoice: ${inv.id}`, inv.amount)} className="hover:bg-blue-50/30 transition-colors group cursor-pointer">
